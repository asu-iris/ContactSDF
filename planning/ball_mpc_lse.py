...
class MPC_IPOPT:
    ...
    def planOnce(self, 
        ...
        nlp_lbw, nlp_ubw = self.nlp_bounds_fn_(
            self.param_.mpc_u_lb_, self.param_.mpc_u_ub_, 
            self.param_.mpc_q_lb_, self.param_.mpc_q_ub_
        )
        ...
        print("Solver: LSE")
        print("mpc solving time:", time.time() - st)
        print('return_status = ', self.ipopt_solver.stats()['return_status'])
        print('mpc_planning = ', opt_u_traj[0, :])
