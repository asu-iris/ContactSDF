...
class Cost:
    ...
    def init_cost_fn(self):
        ...
        # base cost
        base_cost = 0.0 * position_cost + 0.0 * quaternion_cost + 0.0 * grasp_cost + 0.5 * contact_cost
        final_cost = self.param_.p_weighting_ * position_cost + self.param_.q_weighting_ * quaternion_cost

        self.path_cost_fn = cs.Function('path_cost_fn', [x, u, cost_param], [base_cost + 0.1 * control_cost])
