...
from cost.allegro_cube.cost_allegro_cube import Cost
...
class Parameters:
    def __init__(self):
        ...
        # ---------------------------------------------------------------------------------------------
        # 1.
        self.simulator_mode_ = 'allegro_cube'
        ...
        # ---------------------------------------------------------------------------------------------
        self.model_path_ = 'env/xml/planning_mujoco_allegro_cube_env.xml'
        ...
        # ---------------------------------------------------------------------------------------------
        obj_pose = np.array([0.01, -0.0, 0.04, 1, 0, 0, 0])

        self.joint_position = np.array([
            0.25, 0.1, 1.6, 0.219733,
            0.0688058, 0.3, 1.13095, 1.09028,
            0.155662, 0.456852,  1.6, 0.648177,
            0.25, 1.16, 1.5, 1.4
        ])
        ...
        self.target_p_list_ = []
        self.target_p_list_.append(np.array([-0.0, -0.0, 0.0313]))

        self.target_q_list_ = []
        self.target_q_list_.append(rotations.axis_angle_to_quaternion(1.0 * np.pi / 2 * np.array([.0, .0, 1.0])))
        self.target_q_list_.append(rotations.axis_angle_to_quaternion(-1.0 * np.pi / 2 * np.array([.0, .0, 1.0])))
        self.target_q_list_.append(rotations.axis_angle_to_quaternion(1.0 * np.pi / 3 * np.array([.0, .0, 1.0])))
        self.target_q_list_.append(rotations.axis_angle_to_quaternion(-1.0 * np.pi / 3 * np.array([.0, .0, 1.0])))

        target_quaternion = rotations.axis_angle_to_quaternion(1.0 * np.pi / 2 * np.array([.0, .0, 1.0]))
        target_position = np.array([-0.0, -0.0, 0.0313])
        ...
        self.mpc_jointu_lb_ = -0.15
        self.mpc_jointu_ub_ = 0.15
